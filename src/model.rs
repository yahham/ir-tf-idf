...
pub trait Model {
    ...
    fn require_reindexing(&mut self, path: &Path, last_modified: SystemTime) -> Result<bool, ()>;
    fn add_document(
        &mut self,
        path: PathBuf,
...
impl Model for SqliteModel {
    ...
    fn require_reindexing(&mut self, _path: &Path, _last_modified: SystemTime) -> Result<bool, ()> {
        Ok(true)
    }
